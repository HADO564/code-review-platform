...
func (r *mutationResolver) CreateUser(ctx context.Context, input model.NewUser) (*model.User, error) {
    // Step 1: Create a new User model object with the provided details (no UUID or timestamps)
    newUser := &model.User{
        Username: input.Username,  // Use the username provided in the input
        Email:    input.Email,     // Use the email provided in the input
    }

    // Step 2: Insert the new user into the database (ID and timestamps generated by Supabase)
    err := db.CreateUser(ctx, newUser)
    if err != nil {
        return nil, fmt.Errorf("failed to create user in database: %v", err)
    }

    // Step 3: Fetch the created user to return (including auto-generated fields like ID)
    createdUser, err := db.GetUserByEmail(ctx, newUser.Email)  // Assuming you fetch by email to get the full details
    if err != nil {
        return nil, fmt.Errorf("failed to retrieve newly created user: %v", err)
    }

    // Step 4: Return the newly created user with all fields populated
